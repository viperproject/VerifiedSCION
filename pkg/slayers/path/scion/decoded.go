...
func (s *Decoded) DecodeFromBytes(data []byte) (r error) {
	...
	//@ sl.SplitByIndex_Bytes(data, 0, len(data), offset, R42)
	...
	//@ invariant acc(sl.AbsSlice_Bytes(data, 0, offset), R42)
	//@ invariant acc(sl.AbsSlice_Bytes(data, offset, len(data)), R42)
	//@ decreases s.Base.GetNumINF() - i
	for i := 0; i < /*@ unfolding acc(s.Base.Mem(), _) in @*/ s.NumINF; i++ {
		//@ sl.SplitByIndex_Bytes(data, offset, len(data), offset + path.InfoLen, R42)
		//@ sl.Reslice_Bytes(data, offset, offset + path.InfoLen, R42)
		...
		//@ sl.Unslice_Bytes(data, offset, offset + path.InfoLen, R42)
		//@ sl.CombineAtIndex_Bytes(data, 0, offset + path.InfoLen, offset, R42)
		offset += path.InfoLen
	}
	...
	for i := 0; i < /*@ unfolding acc(s.Base.Mem(), R2) in @*/ s.NumHops; i++ {
		//@ sl.SplitByIndex_Bytes(data, offset, len(data), offset + path.HopLen, R42)
		//@ sl.Reslice_Bytes(data, offset, offset + path.HopLen, R42)
		...
		//@ sl.Unslice_Bytes(data, offset, offset + path.HopLen, R42)
		//@ sl.CombineAtIndex_Bytes(data, 0, offset + path.HopLen, offset, R42)
		offset += path.HopLen
	}
	//@ sl.CombineAtIndex_Bytes(data, 0, len(data), offset, R42)
	//@ fold s.Mem(data)
	return nil
}
